...
are licensed differenttly from the IDE.
...
and an include file that adds all icons to an image list. Therefore be carefull 
to rename icons in the SVG file as the IDE might not compile.

Loading icons in Easy80-IDE
---------------------------
Loading an icon in the IDE is very simple. Simply use the filename without the 
extension e.g.
...
At startup the IDE will load all icons into an image list by executing the 
following procedure:
